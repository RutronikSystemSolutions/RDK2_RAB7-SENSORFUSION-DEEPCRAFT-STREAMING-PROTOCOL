...
int8_t bmi3_interface_init_i2c(struct bmi3_dev *dev, void* i2c_bus)
{
    if (dev != NULL)
    {
		printf("BMI2xx I2C Interface Initialized.\r\n");
		bmi323_i2c = (cyhal_i2c_t*) i2c_bus;
...
static BMI3_INTF_RET_TYPE bmi3_i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
	...
	result = cyhal_i2c_master_write( bmi323_i2c, (uint16_t)dev_addr, &reg_addr, 1, 10, false );
    if (result != CY_RSLT_SUCCESS)
    {
    	return BMI3_E_COM_FAIL;
    }

	result = (int8_t)cyhal_i2c_master_read(bmi323_i2c,(uint16_t)dev_addr, reg_data, (uint16_t)len, 10, true);
	if (result != CY_RSLT_SUCCESS)
	{
		 return BMI3_E_COM_FAIL;
	}
...
static BMI3_INTF_RET_TYPE bmi3_i2c_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
    ...
    /*Write data to I2C*/
	result = cyhal_i2c_master_write(bmi323_i2c, (uint16_t)dev_addr, i2c_data, len+1, 100, true);
    if (result != CY_RSLT_SUCCESS)
    {
    	free(i2c_data);
    	return BMI3_E_COM_FAIL;
