...
static bool _init_hw(dev_bmm350_t *dev, cyhal_i2c_t* i2c)
{
    ...
    if (rslt != BMM350_OK)
    {
    	printf("bmm350_get_pmu_cmd_status_0 error : %d \r\n", rslt);
        return false;
    }
    ...
    if (rslt != BMM350_OK)
    {
    	printf("bmm350_get_regs error : %d \r\n", rslt);
        return false;
    }
    ...
    if (rslt != BMM350_OK)
    {
    	printf("bmm350_configure_interrupt error : %d \r\n", rslt);
        return false;
    }
...
static bool _config_hw(dev_bmm350_t *dev, int rate)
{
    ...
    printf("bmm350: Configured device. rate=%d Hz, frames/chunk=%d\r\n", rate, dev->frames_target);
...
static bool _configure_streams(protocol_t* protocol, int device, void* arg)
{
    ...
    printf("dev_bmm350 configure streams\r\n");
...
*
*******************************************************************************/
int dev_bmm350_register(protocol_t* protocol, cyhal_i2c_t* i2c)
{
    int status;
